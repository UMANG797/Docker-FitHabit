...
import { Trophy, Flame, Target, Star, Award, Zap, Crown, TrendingUp, CheckCircle, Calendar, Gift, Plus, X, RotateCcw } from 'lucide-react';
...
const BADGE_SYSTEM = {
  ...
  'streak_14': {
    ...
    description: 'Conquered 14 consecutive days',
  ...
  'streak_60': {
    icon: Zap,
    name: 'Unstoppable',
    description: 'Reached an incredible 60-day streak',
  ...
  'streak_100': {
    icon: Crown,
    name: 'Century Club',
    description: 'Legendary 100-day achievement',
  ...
  'comeback': {
    icon: TrendingUp,
    name: 'Comeback King',
    description: 'Restarted after a break and hit 5 days',
...
function HabitCard({ habit, onUpdated, onDeleted }) {
  ...
  const currentBadges = calculateBadges(habit.streak);
  const nextBadge = Object.entries(BADGE_SYSTEM).find(
    ([id, badge]) => badge.requirement !== 'special' && habit.streak < badge.requirement
  );
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all border-2 border-gray-100 p-6 relative overflow-hidden group">
      ...
      <div className="mb-5">
        ...
        <p className="text-gray-600 text-base leading-relaxed">{habit.description}</p>
      </div>
      ...
      {currentBadges.length > 0 ? (
        ...
        <div className="mb-5 p-4 bg-gradient-to-br from-gray-50 to-blue-50 rounded-xl border-2 border-dashed border-gray-300">
          ...
          <div className="text-sm text-gray-500">
            {nextBadge ? (
              <>
                <span className="font-semibold text-blue-600">{nextBadge[1].requirement - habit.streak} more days</span> to unlock "{nextBadge[1].name}"
              </>
      ...
      <div className="flex items-center gap-4 mb-5 text-sm">
        ...
        {nextBadge && (
          <div className="flex items-center gap-2 px-3 py-2 bg-blue-50 rounded-lg border border-blue-200">
            ...
            <span className="text-gray-700">Next: <strong className="text-blue-700">{nextBadge[1].requirement}</strong></span>
          </div>
        )}
      </div>
...
export default function App() {
  ...
  const totalBadges = habits.reduce((sum, h) => {
    const badges = [3, 7, 14, 30, 60, 100].filter(req => h.streak >= req);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 py-8 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <NewHabitForm onCreated={addHabit} />
